  ...
  <link rel="stylesheet" property="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/css/bootstrap.min.css">
    ...
    <section class="inner-page">
      <div class="container">
        ...
        <h3 style="margin-top: 5%;">Commission List</h3>
        ...
        <table class="table table-borderless">
          ...
          <tbody>
          	...
          	<c:forEach var="buy" items="${buy}">
	            <tr>
	              <td>${buy.buy_no}</td>
	              <td><a href="BuyServlet?command=buy-detail&buy_no=${buy.buy_no}&pageno=${pageno}">${buy.buy_title}</a></td>
	              <td>${buy.id}</td>
	              <td>${buy.buy_date}</td>
        ...
        <div class="text-center">          
          <ul class="pagination">
          	...
          	<c:if test="${startpage > 1}">
          		<li><a href="BuyServlet?command=buy&pageno=1">처음으로</a></li>
          	</c:if>
          	
          	<c:if test="${pageno > 1}">
          		<li><a href="BuyServlet?command=buy&pageno=${pageno-1}">이전</a></li>
          	</c:if>
			
			<c:forEach var="i" begin="${startpage}" end="${endpage}">
				<li><a href="BuyServlet?command=buy&pageno=${i}">${i}</a></li>
			</c:forEach>
  	
  			<c:if test="${pageno < totalpage}">
  			   <li><a href="BuyServlet?command=buy&pageno=${pageno+1}">다음</a></li>
  			</c:if>
  	
  			<c:if test="${endpage < totalpage}">
  				<li><a href="BuyServlet?command=buy&pageno=${totalpage}">끝으로</a></li>
  			</c:if>
